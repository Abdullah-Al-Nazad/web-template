...
export default function GalleryList() {
    ...
    useEffect(() => {
        axios.get("http://localhost/React/project/src/admin/database/gallery/galleryget.php")
            .then((res) => setData(res.data))
    }, [])
    const deletes = (id) => {
        axios.get(`http://localhost/React/project/src/admin/database/gallery/gallerydelete.php?id=${id}`)
            .then((res) => {
    ...
    return (
        <div>
            <NavLink to={`/admin/gallerylist/add`}>
                    ...
                    {data.map((d, i) => {
                        return (
                            <tr key={i}>
                                ...
                                <td>{d.date}</td>
                                <td>
                                    <NavLink to={`/admin/gallerylist/update/${d.id}`}>
